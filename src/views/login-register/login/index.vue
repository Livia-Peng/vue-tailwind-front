  ...
  <div class="relative h-screen bg-white dark:bg-zinc-800 text-center xl:bg-zinc-200">
    ...
    <div
      ...
      <vee-form @submit="onLoginHandler">
        ...
        <vee-error-message class="text-sm text-red-600 block mt-0.5 text-left" name="username"> </vee-error-message>
        <vee-field
          class="dark:bg-zinc-800 dark:text-zinc-400 border-b-zinc-400 border-b-[1px] w-full outline-0 pb-1 px-1 text-base focus:border-b-main dark:focus:border-b-zinc-200 xl:dark:bg-zinc-900"
          name="password"
        ...
        <vee-error-message class="text-sm text-red-600 block mt-0.5 text-left" name="password"> </vee-error-message>
        ...
        <m-button class="w-full text-sm dark:bg-zinc-900 xl:dark:bg-zinc-800" :loading="loading" :isActiveAnim="false">
          登录
        </m-button>
      </vee-form>
  ...
  const onLogin = async () => {
    ...
    try {
      await store.dispatch('user/login', {
        ...loginForm.value,
        loginType: commonConfig.loginTypes.username
      })
    } finally {
      loading.value = false
    }
