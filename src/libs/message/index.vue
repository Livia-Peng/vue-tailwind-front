<template>
  <transition name="down" @after-leave="destroy">
    <div
      ...
      <m-svg-icon :name="styles[type].icon" :fillClass="styles[type].fillClass" class="h-1.5 w-1.5 mr-1.5"></m-svg-icon>
  ...
  const props = defineProps({
    ...
    type: {
      ...
      validator(val) {
        ...
        if (!result) {
          throw new Error(`message type 必须是 ${typeEnum.join('、')} 中的一个`)
        }
        return result
      }
    },
