...
<script setup>
  ...
  const useContainerWidth = () => {
    const { paddingLeft, paddingRight } = window.getComputedStyle(containerTarget.value, null)
    // 容器左边距
    containerLeft.value = parseFloat(paddingLeft)
    // 容器宽度
    containerWidth.value = containerTarget.value.offsetWidth - containerLeft.value - parseFloat(paddingRight)
  }
  ...
  const useColumnWidth = () => {
    // 获取容器宽度
    useContainerWidth()
    columnWidth.value = (containerWidth.value - (props.column - 1) * props.columnSpacing) / props.column
  }
  onMounted(() => {
    useColumnWidth()
  })
